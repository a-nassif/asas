...
INTENTIONAL_MODULE_EXPORTS = {"service"}
...
def test_declares_all():
    ...
    assert hasattr(asas_notifications, "__all__"), "asas_notifications must declare __all__"
    assert asas_notifications.__all__, "asas_notifications.__all__ must not be empty"


def test_every_exported_name_resolves():
    """A name in ``__all__`` that does not exist is a typo that surfaces only on
    ``import *`` — or never."""
    missing = [n for n in asas_notifications.__all__ if not hasattr(asas_notifications, n)]
    assert not missing, f"asas_notifications.__all__ names that do not resolve: {missing}"
...
def test_contract_names_are_callable():
    ...
    broken = [
        ...
        if hasattr(asas_notifications, n) and not callable(getattr(asas_notifications, n))
    ]
    assert not broken, (
        f"asas_notifications exposes contract names that are not callable: {broken}. "
...
def test_module_exports_are_intentional():
    ...
    exported = {
        n for n in asas_notifications.__all__
        if isinstance(getattr(asas_notifications, n), types.ModuleType)
    }
    unexpected = exported - INTENTIONAL_MODULE_EXPORTS
    assert not unexpected, (
        f"asas_notifications exports undeclared modules: {sorted(unexpected)}. If that is "
...
def test_version_is_exported():
    ...
    assert isinstance(asas_notifications.__version__, str)
    assert asas_notifications.__version__
