...
_BASELINE = "0001"
_SENTINEL_TABLE = "background_job"
...
def _assert_adoptable(inspector) -> None:
    ...
    if absent:
        raise RuntimeError(
            f"asas-jobs cannot adopt the existing {_SENTINEL_TABLE!r} table: the "
            f"baseline's other tables are missing {absent}. This database holds a "
            f"partial asas-jobs schema; stamping it would record the baseline as "
    ...
    if missing:
        raise RuntimeError(
            f"asas-jobs cannot adopt the existing {_SENTINEL_TABLE!r} table: it is "
            f"missing the baseline columns {sorted(missing)}. This database already "
            f"contains an unrelated table named {_SENTINEL_TABLE!r}, so asas-jobs "
