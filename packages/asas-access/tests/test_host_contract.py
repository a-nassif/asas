...
def test_declares_all():
    ...
    assert hasattr(asas_access, "__all__"), "asas_access must declare __all__"
    assert asas_access.__all__, "asas_access.__all__ must not be empty"


def test_every_exported_name_resolves():
    """A name in ``__all__`` that does not exist is a typo that surfaces only on
    ``import *`` — or never."""
    missing = [n for n in asas_access.__all__ if not hasattr(asas_access, n)]
    assert not missing, f"asas_access.__all__ names that do not resolve: {missing}"
...
def test_contract_names_are_callable():
    ...
    broken = [
        ...
        if hasattr(asas_access, n) and not callable(getattr(asas_access, n))
    ]
    assert not broken, (
        f"asas_access exposes contract names that are not callable: {broken}. "
...
def test_module_exports_are_intentional():
    ...
    exported = {
        n for n in asas_access.__all__
        if isinstance(getattr(asas_access, n), types.ModuleType)
    }
    unexpected = exported - INTENTIONAL_MODULE_EXPORTS
    assert not unexpected, (
        f"asas_access exports undeclared modules: {sorted(unexpected)}. If that is "
...
def test_version_is_exported():
    ...
    assert isinstance(asas_access.__version__, str)
    assert asas_access.__version__
