...
VERSION_TABLE = "alembic_version_asas_search"
_BASELINE = "0001"
_SENTINEL_TABLE = "search_document"
...
def _assert_adoptable(inspector) -> None:
    ...
    if absent:
        raise RuntimeError(
            f"asas-search cannot adopt the existing {_SENTINEL_TABLE!r} table: the "
            f"baseline's other tables are missing {absent}. This database holds a "
            f"partial asas-search schema; stamping it would record the baseline as "
